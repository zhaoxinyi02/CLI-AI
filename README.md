...
CLI-AI/
├── README.md                       # 项目说明文档
├── ENHANCED_FEATURES.md            # 增强功能详细文档 (v2.2.1-2.2.3)
...
├── ai_command_parser.py            # AI 命令解析器 (v2.1, 增强 v2.2)
├── context_manager.py              # 系统环境上下文管理器 (v2.2.1)
...
├── test_context_manager.py         # 上下文管理器测试 (v2.2.1)
├── test_enhanced_parser.py         # 增强功能测试 (v2.2.1-2.2.3)
├── demo_enhanced_features.py       # 增强功能演示 (v2.2.1-2.2.3)
└── prompts/                        # AI 提示词模板目录 (v2.1, 扩展 v2.2.3)
    ├── .gitkeep
    ├── command_generation.txt      # 通用命令生成提示词
    ├── file_operations.txt         # 文件操作场景提示词 (v2.2.3)
    ├── system_management.txt       # 系统管理场景提示词 (v2.2.3)
    ├── network_operations.txt      # 网络操作场景提示词 (v2.2.3)
    └── text_processing.txt         # 文本处理场景提示词 (v2.2.3)
```
...
## 增强功能 (v2.2.1 - v2.2.3) | Enhanced Features

### 2.2.1 系统环境上下文感知

CLI-AI 现在可以感知系统环境，并利用上下文信息提供更精准的命令建议。

**功能特点**：
- 🖥️ 自动收集系统信息（工作目录、用户名、操作系统、架构）
- 🔄 智能上下文缓存和刷新机制
- 📊 提供多种格式的上下文信息输出
...
AI 命令解析器现在能够智能提取和验证参数。

**功能特点**：
- 🔍 从自然语言中提取文件名、路径、数字、选项
- ✅ 验证文件和路径是否存在
- 🛠️ 自动补全路径（相对路径转绝对路径）
- 🏠 支持 `~` 路径扩展
...
print(params['files'])  # ['test.txt', 'backup.txt']

# 路径补全
path = parser.autocomplete_path("~/documents")
...
print(info['scenario'])      # 'file_operations'
print(info['prompt_file'])   # 'prompts/file_operations.txt'
```
...
更多详细信息，请参阅 [ENHANCED_FEATURES.md](ENHANCED_FEATURES.md)。
...
### v2.2.1-2.2.3 (2026-01-10)
- ✨ 新增系统环境上下文感知功能 (`context_manager.py`)
  - 自动收集工作目录、用户名、操作系统、系统架构
  - 提供多种格式的上下文信息输出
- 🔍 增强 AI 命令解析器的参数提取与验证功能
  - 智能提取文件名、路径、数字、选项参数
  - 验证文件和路径是否存在
  - 支持路径自动补全（相对路径转绝对路径）
- 🎯 新增场景化 Prompt 优化
  - 创建 4 个场景专用提示词模板（文件操作、系统管理、网络操作、文本处理）
  - 实现智能场景检测和自动 Prompt 选择
- 🧪 添加完整测试套件
  - `test_context_manager.py` - 上下文管理器测试（10个测试）
  - `test_enhanced_parser.py` - 增强功能测试（28个测试）
- 📚 新增详细文档 `ENHANCED_FEATURES.md`
- 🎨 创建功能演示脚本 `demo_enhanced_features.py`
